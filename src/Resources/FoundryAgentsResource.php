...
final class FoundryAgentsResource extends FoundryScopedResource
{
     ...
     * @return array<string, mixed>
     */
    public function replace(string $agentName, array|UpdateAgentPayload $body): array
    {
        $payload = $body instanceof UpdateAgentPayload
            ? $body
            : new GenericJsonPayload($body);
        $response = $this->dispatchFoundry(new ReplaceAgent($agentName, $payload));

        return $this->jsonArray($response);
    }

    /**
     * @param  array<string, mixed>|AgentVersionRoutingPayload  $versionSelector
     * @return array<string, mixed>
     */
    public function setVersionRouting(string $agentName, array|AgentVersionRoutingPayload $versionSelector): array
    {
        $payload = $versionSelector instanceof AgentVersionRoutingPayload
            ? $versionSelector
            : new AgentVersionRoutingPayload($versionSelector);
        $response = $this->dispatchFoundry(new SetAgentVersionRouting($agentName, $payload));
