...
final class AzureClient
{
    ...
    /** @var array<string, StorageQueueConnector> */
    private array $storageQueueConnectors = [];
    ...
    public function storageQueue(string $accountName, ?string $accountKey = null): StorageQueueConnector
    {
        $cacheKey = $accountName.':'.($accountKey ?? '');

        return $this->storageQueueConnectors[$cacheKey] ??= new StorageQueueConnector(
            $this->config,
            $this->tokens,
            $this->fetcher,
            $accountName,
