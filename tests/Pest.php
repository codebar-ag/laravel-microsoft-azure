...
/**
 * A fixture scoped to the `live/` namespace, kept separate from hand-crafted fixtures
 * committed for offline unit tests (e.g. tests/Unit/DataAndFixturesTest.php's
 * "committed saloon fixture" for get-resource-group) so live recordings never clobber
 * or get shadowed by them. Deletes any existing recording first so MICROSOFT_AZURE_RECORD_FIXTURES=true
 * actually refreshes stale data instead of replaying whatever was last captured.
 */
function liveFixture(string $name): MicrosoftAzureFixture
{
    $path = __DIR__.'/Fixtures/saloon/live/'.$name.'.json';

    if (integrationFixturesEnabled() && file_exists($path)) {
        unlink($path);
    }

    return new MicrosoftAzureFixture('live/'.$name);
}
...
function configureIntegrationRecording(): void
{
    ...
    Azure::instance()->arm()->withMockClient(new MockClient([
        ListSubscriptions::class => liveFixture('list-subscriptions'),
        GetSubscription::class => liveFixture('get-subscription'),
        CancelSubscription::class => liveFixture('cancel-subscription'),
        CreateOrUpdateResourceGroup::class => liveFixture('create-resource-group'),
        GetResourceGroup::class => liveFixture('get-resource-group'),
        ListResourceGroups::class => liveFixture('list-resource-groups'),
        DeleteResourceGroup::class => liveFixture('delete-resource-group'),
        CreateOrUpdateSubscriptionAlias::class => liveFixture('create-subscription-alias'),
        GetSubscriptionAlias::class => liveFixture('get-subscription-alias'),
        ListSubscriptionAliases::class => liveFixture('list-subscription-aliases'),
        DeleteSubscriptionAlias::class => liveFixture('delete-subscription-alias'),
        // Fallback for every other ARM request class: derive the fixture name from the
        // request's own class name so newly added resources record/replay automatically
        // without this list needing to be kept in sync by hand.
        ArmConnector::class => function (PendingRequest $pendingRequest): MicrosoftAzureFixture {
            $shortName = class_basename($pendingRequest->getRequest());

            return liveFixture(Str::kebab($shortName));
        },
    ]));
}
 ...
 * @param  array<int, MockResponse>|array<class-string, MockResponse>  $responses
 */
function clientWithStorageQueueMock(array $responses, string $accountName = 'mystorageacct', ?string $accountKey = null): AzureClient
{
    $client = clientWithSeededToken();

    $client->storageQueue($accountName, $accountKey)->withMockClient(new MockClient($responses));
