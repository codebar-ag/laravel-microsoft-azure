...
it('resolves cognitive services request endpoints and api-version query', function (callable $factory, string $endpoint, ApiVersion $apiVersion): void {
    /** @var Request $request */
    $request = $factory();
...
it('omits properties as a json object rather than an empty array when unset', function (): void {
    // Azure rejects an empty PHP array serialized as JSON `[]` where it
    // expects an object `{}` for `properties` — guards against that regression
    // (caught live: ARM rejected FoundryProjectPayload's default `[]`).
    $accountRequest = new CreateOrUpdateCognitiveServicesAccount(
        subscriptionId: 'sub-1',
        resourceGroupName: 'rg-test',
        accountName: 'aif-test',
        payload: new CognitiveServicesAccountPayload(location: 'westeurope'),
    ...
    $projectRequest = new CreateOrUpdateFoundryProject(
        subscriptionId: 'sub-1',
        resourceGroupName: 'rg-test',
        accountName: 'aif-test',
        projectName: 'proj-1',
        payload: new FoundryProjectPayload('westeurope'),
    );

    expect($accountRequest->body()->all())->not->toHaveKey('properties')
        ->and($projectRequest->body()->all())->not->toHaveKey('properties');
});

it('sets a system-assigned identity on the account body when requested', function (): void {
    $request = new CreateOrUpdateCognitiveServicesAccount(
        subscriptionId: 'sub-1',
        resourceGroupName: 'rg-test',
        accountName: 'aif-test',
        payload: new CognitiveServicesAccountPayload(location: 'westeurope', identityType: 'SystemAssigned'),
    );

    expect($request->body()->all())->toHaveKey('identity', ['type' => 'SystemAssigned']);
});

it('sets a system-assigned identity on the project body when requested', function (): void {
    $request = new CreateOrUpdateFoundryProject(
        subscriptionId: 'sub-1',
        resourceGroupName: 'rg-test',
        accountName: 'aif-test',
        projectName: 'proj-1',
        payload: new FoundryProjectPayload('westeurope', identityType: 'SystemAssigned'),
    );
