...
dataset('storage request endpoints', [
    ...
    'CreateOrUpdateQueue' => [
        fn () => new CreateOrUpdateQueue('sub-1', 'rg-test', 'sa1', 'orders'),
        '/subscriptions/sub-1/resourceGroups/rg-test/providers/Microsoft.Storage/storageAccounts/sa1/queueServices/default/queues/orders',
        ApiVersion::ARM_STORAGE,
    ],
    'DeleteQueue' => [
        fn () => new DeleteQueue('sub-1', 'rg-test', 'sa1', 'orders'),
        '/subscriptions/sub-1/resourceGroups/rg-test/providers/Microsoft.Storage/storageAccounts/sa1/queueServices/default/queues/orders',
        ApiVersion::ARM_STORAGE,
    ],
]);

it('resolves storage request endpoints and api-version query', function (callable $factory, string $endpoint, ApiVersion $apiVersion): void {
    /** @var Request $request */
    $request = $factory();
...
it('builds create queue body with metadata as a json object even when empty', function (): void {
    $withMetadata = new CreateOrUpdateQueue('sub-1', 'rg-test', 'sa1', 'orders', ['owner' => 'billing']);
    $withoutMetadata = new CreateOrUpdateQueue('sub-1', 'rg-test', 'sa1', 'orders');

    // Azure rejects `[]` for an object-typed field — guards against that regression.
    expect((string) $withMetadata->body())->toBe('{"properties":{"metadata":{"owner":"billing"}}}')
        ->and((string) $withoutMetadata->body())->toBe('{"properties":{"metadata":{}}}');
});
