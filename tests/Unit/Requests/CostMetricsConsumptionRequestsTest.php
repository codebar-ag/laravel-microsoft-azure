...
it('resolves endpoint and api-version for cost/metrics/consumption requests', function (callable $factory, string $endpoint, ApiVersion $apiVersion): void {
    /** @var Request $request */
    $request = $factory();

    expect($request->resolveEndpoint())->toBe($endpoint)
        ->and($request->query()->all()['api-version'])->toBe($apiVersion->value());
...
it('adds $filter to ListUsageDetails query when provided', function (): void {
    ...
    expect($query)->toHaveKey('$filter', "properties/usageStart ge '2026-01-01'")
        ->and($query['api-version'])->toBe(ApiVersion::ARM_CONSUMPTION->value());
});
