...
use CodebarAg\MicrosoftAzure\Requests\KeyVault\ListSecretVersions;
...
it('lists key vault secret versions via vault resource gateway', function (): void {
    $client = clientWithKeyVaultMock([
        ListSecretVersions::class => MockResponse::make(body: [
            'value' => [secretIdentifierFixture()],
        ]),
    ]);

    $versions = $client->vault('myvault')->secrets()->versions('webhook-token');

    expect($versions)->toHaveCount(1)
        ->and($versions->first()?->enabled)->toBeTrue();
});
...
it('maps api version constants', function (): void {
    expect(ApiVersion::ARM_RESOURCES->value())->toBe('2025-04-01')
        ->and(ApiVersion::KEY_VAULT->value())->toBe('2025-07-01')
        ->and(ApiVersion::GRAPH->value())->toBe('v1.0');
});
