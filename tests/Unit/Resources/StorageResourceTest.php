...
it('reaches the storage queue data-plane gateway from a storage account resource', function (): void {
    $client = clientWithArmMock([
        GetStorageAccount::class => MockResponse::make(body: storageAccountFixture()),
    ]);

    $account = (new StorageAccountsResource($client, 'sub-1', 'rg-test'))->account('sa1');

    expect($account->queue('orders'))->toBeInstanceOf(StorageQueueResource::class);

    // sanity: the queue gateway is wired to the storage-queue connector, not ARM
    $client->storageQueue('sa1')->withMockClient(new MockClient([
        SendMessage::class => MockResponse::make(body: '<QueueMessagesList><QueueMessage><MessageId>msg-1</MessageId></QueueMessage></QueueMessagesList>'),
    ]));

    expect($account->queue('orders')->sendMessage('hello')->messageId)->toBe('msg-1');
});

it('creates and deletes a queue via the arm control plane', function (): void {
    $client = clientWithArmMock([
        CreateOrUpdateQueue::class => MockResponse::make(body: [
            'id' => '/subscriptions/sub-1/resourceGroups/rg-test/providers/Microsoft.Storage/storageAccounts/sa1/queueServices/default/queues/orders',
            'name' => 'orders',
            'properties' => ['metadata' => ['owner' => 'billing']],
        ]),
        DeleteQueue::class => MockResponse::make(body: '', status: 204),
    ]);

    $queues = (new StorageAccountsResource($client, 'sub-1', 'rg-test'))->account('sa1')->queues();

    $queue = $queues->createOrUpdate('orders', ['owner' => 'billing']);
    $queues->delete('orders');

    expect($queue->name)->toBe('orders')
        ->and($queue->metadata)->toBe(['owner' => 'billing']);
});
